...
class RomanCalendar1962():
	...
	feast_day = None 								# the current feast-day
	...
	def __init__(self,year=datetime.now().year):
		...
		self.easter = easter.easter(self.year) 
		self._set_christmas()
		...
		self.liturgical_season = self.get_season()
		#self.feast_day = self.get_feast_day()
...
class PiusXPsalter():
	_nocturn1 = ( '1,2,3', '13,14,16', '34i,34ii,34iii', '44i,44ii,45', '61,65i,65ii', '77i,77ii,77iii', '104i,104ii,104iii' )
	_nocturn2 = ( '8,9i,9ii', '17i,17ii,17iii', '36i,36ii,36iii', '47,48i,48ii', '67i,67ii,67iii', '77iv,77v,77vi', '105i,105ii,105iii' )
	_nocturn3 = ( '9iii,9iv,10', '19,20,29', '37i,37ii,38', '49i,49ii,50', '68i,68ii,68iii', '78,80,82', '106i,106ii,106iii' )
	_lauds1 = ('92,99,62,Benedicite_omnia,148', '46,5,28,Benedictus_es,116', '95,42,66,Magnus_es,134', '96,64,100,Incipite_Domino,145', '97,89,35,Audite_verbum_Domini,146', '98,142,84,Vere_tu_es,147', '149,91,63,Miserere_nostri,150' )
	_lauds2 = ()
	_prime = ()
	_terce = ()
	_sext = ()
...
class Breviary():
	...
	def __init__(self,year=datetime.now().year):
		self.calendar= RomanCalendar1962()
	...
	def season(self, thistime=date(datetime.now().year, datetime.now().month, datetime.now().day)):
		if self.calendar.get_feast() is not 'Feria':
			return "%s (feast of %s)"%(self.calendar.get_season(),self.calendar.get_feast())
		else:
			return "%s (%s)"%(self.calendar.get_season(),self.calendar.get_feast())
	...
	def full_office(self, thistime=date(datetime.now().year, datetime.now().month, datetime.now().day)):
		"""takes an optional datetime.time parameter, or uses the current time to 
			return a string with the office, season and feast
		"""
		if self.calendar.feast_day is not 'Feria':
			return "office of %s for %s in %s (feast of %s)"%(self.office(), datetime.now().strftime("%A"), self.calendar.get_season(),self.calendar.get_feast())
		else:
			return "office of %s for %s in %s (%s)"%(self.office(), datetime.now().strftime("%A"), self.calendar.get_season(),self.calendar.get_feast())
