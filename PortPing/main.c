 ...
 *
 * TODO/ROADMAP:
 * 	+ need to interrupt connect()'s that last longer than 1s. (v0.0.2)
 * 	+ time calls to connect() for output (v0.0.2)
 *	+ write portPingUDP() (v0.0.3)
 * 	+ write man page/general documentation
 *  + include support to fragment connect's over large and small packets
 *
 */
...
int portPingUDP( const char* target, const char* targetport )
{
	//TODO: for v0.0.3
...
int portPingTCP( const char* target, const char* targetPort )
{
	...
	if( timeEnd.tv_sec != timeStart.tv_sec )
		printf( "%i", timeEnd.tv_sec - timeStart.tv_sec );
		
	timeDiff = timeEnd.tv_usec - timeStart.tv_usec;
	//TODO: timeDiff will need to be calculated differently if the connect() spanned a second
	
    if( -1 == status )
			printf( ".%is -- FAILED to connect to %s:%s/tcp\n", timeDiff, target, targetPort );
	else
			printf( ".%is -- connected to %s:%s/tcp\n", timeDiff, target, targetPort );
	...
	if( opt_count )
	{
		for( x = 1; x <= opt_count; x++ )
		{
			if( strcmp( opt_proto, "tcp" ) == 0)
					(void)portPingTCP( opt_host, opt_port );
			else if( strcmp( opt_proto, "udp" ) == 0)
					(void)portPingUDP( opt_host, opt_port );
			sleep( opt_delay );
		}
	} else {
		for(;;)
