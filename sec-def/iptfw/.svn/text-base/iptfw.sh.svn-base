...
# 
# TODO/ROADMAP: 
#	- add web-based interface to mangle trusted users (v.0.3.0)
#	- add egress filtering (v.0.4.0)
#	- add support for spoofed packets in 'gateway' mode (v.0.4.0)
...
INT_IP=""
EXT_IP="192.168.2.2"
CONFD_INTS=`/sbin/ifconfig -a | grep inet | awk {'print $2'} | sed s/addr://`
...
# trusted services
TRUSTEDSERVICES="21 22 5901"

# outbound services
TCP_OUT="80 25"
UDP_OUT="53"

# trusted user ID's that are allowed to bypass packet filters.
# 	refer to /etc/passwd for id's, or check with `man id`
TRUSTED_UIDS="500"
...
PERMTRUSTED="192.168.2.106/32 65.125.46.101/32 63.174.120.23/32"
TRUSTED=`$LSTRUSTED`
TRUSTED="$PERMTRUSTED $TRUSTED" #this set of strings is UGLY... can we fix it?
