<?php
/* 
 * index.php v.0.2.0 by C.J. Steele <csteele@good-sam.com>
 * 
 */
?>
<html>
...
switch( $f ){
	...
	case 'odrill':
		...
		if( $d )
		{
			...
			$dates_q = "select distinct date from others where message like '%$p%'"; 
	...
	case 'top10':
		...
		{
			print "<li><a href=index.php?f=drilldown&q=$ip>$ip</a> ($ct)</li>\n";
		}
		print "</ol>"; 
	...
	case 'drilldown':
		if( $q ){
			...
			$dates_q = "select distinct date from blocked where src='$q'"; 
			$dates_r = mysql_query( $dates_q );
	...
	case 'pdrill':
		...
		$dates_q = "select distinct date from blocked where dstprt='$q'";
		$dates_r = mysql_query( $dates_q );
	...
	case 'viewday':
		...
		if( $q ){
			...
			while( list( $dstprt, $proto, $dstprt_ct ) = mysql_fetch_row( $top10_attprt_r ) )
			{
				...
				print "<li><a href=\"$this?f=pdrill&q=$dstprt&proto=$proto&d=$q&src=int\">$dstprt/$proto</a> ($service_guess) - $dstprt_ct</li>\n";
			}
			print "</ol>\n</td><td valign=top>";
			...
			while( list( $dstprt, $proto, $dstprt_ct ) = mysql_fetch_row( $top10_attprt_r ) )
			{
				$service_guess = getservbyport ( $dstprt, $proto ); 
				if( $service_guess == "" ){ $service_guess = "<a target=new href=\"http://www.google.com/search?q=$dstprt+$proto\">???</a>"; }
				print "<li><a href=\"$this?f=pdrill&q=$dstprt&proto=$proto&d=$q&src=ext\">$dstprt/$proto</a> ($service_guess) - $dstprt_ct</li>\n";
			}
			print "</ol>";
			...
			while( list( $int_ip, $int_ct ) = mysql_fetch_row( $int_r ) )
			{
				...
				if( $audit_ct )
				{
					print "<li><a href=\"index.php?f=drilldown&q=$int_ip&d=$q\">$int_ip</a> [<a href=\"javascript:onclick=popupwindow('audit.php?query=$int_ip&ipomac=ip', 'audit', '425', '400' )\">dhcp</a> | <a href=\"javascript:onclick=popupwindow('portscan.php?query=$int_ip', 'audit', '425', '400' )\">scan</a>] &nbsp;($int_ct)</li>\n";
				} else {
					print "<li><a href=\"index.php?f=drilldown&q=$int_ip&d=$q\">$int_ip</a> [<a href=\"javascript:onclick=popupwindow('portscan.php?query=$int_ip', 'audit', '425', '400' )\">scan</a>] ($int_ct)</li>\n";
				}
			}
			...
			{
				print "<li><a href=\"index.php?f=drilldown&q=$ext_ip&d=$q\">$ext_ip</a> [<a href=\"javascript:onclick=popupwindow('resolve.php?query=$ext_ip', 'audit', '425', '400' )\">dns</a> | <a href=\"javascript:onclick=popupwindow('portscan.php?query=$ext_ip', 'audit', '425', '400' )\">scan</a> ] ($ext_ct)</li>\n";
			}
			...
			{
				print "<li><a href=\"javascript:onclick=popupwindow('index.php?f=odrill&p=$m', 'odrill', '750', '400' )\">details</a> $m</li>\n";
			}
			print "</td></tr></table></div>"; 
		} else {

			print "<h3>No date specified, click <a href=index.php>here</a>.</h3>\n";
	...
	default:
		...
		print "<h1>F.W.A.P (FireWall Analysis Program)</h1>\n";

		$date_q = "select distinct date from blocked"; 
		$date_r = mysql_query( $date_q );
		print "<hr><h2>Daily Reports</h2><form method=post action=index.php><input type=hidden name=f value=\"viewday\">
		...
		<a href=\"javascript:onclick=popupwindow('trends.php?q=both', 'bothtrends', '400', '400' )\">All Attack Trends</a> | 
		<a href=\"javascript:onclick=popupwindow('trends.php?q=internal', 'inttrends', '400', '400' )\">Internal Attack Trends</a> | 
		<a href=\"javascript:onclick=popupwindow('trends.php?q=external', 'exttrends', '400', '400' )\">External Attack Trends</a>\n";
