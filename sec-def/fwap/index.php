<?php
/* 
 * index.php v.1.0.1 by C.J. Steele <csteele@good-sam.com>
 * 
 */
?>
...
$url = "http://frank.corp.good-sam.com/sec/fwap2"; 
$this = "index.php";
...
$link = mysql_connect( "localhost", "root", "" ) or die ( "Couldn't connect to database server." );
mysql_select_db( "fw2" ) or die ( "Couldn't select the database." );
	...
	case 'viewday':
		if( $q ){
			...
			while( list( $dstprt, $proto, $dstprt_ct ) = mysql_fetch_row( $top10_attprt_r ) )
			{
				...
				if( $service_guess == "" ){ $service_guess = "<a target=new href=\"http://isc.sans.org/port_details.php?port=$dstprt\">???</a>"; }
				print "<li><a href=\"$url/$this?f=pdrill&q=$dstprt&proto=$proto&d=$q&src=int\">$dstprt/$proto</a> (<a target=new href=http://isc.sans.org/port_details.php?port=$dstprt>$service_guess</a>) - $dstprt_ct</li>\n";
			}
			print "</ol>\n</td><td valign=top>";
			...
			while( list( $dstprt, $proto, $dstprt_ct ) = mysql_fetch_row( $top10_attprt_r ) )
			{
				$service_guess = getservbyport ( $dstprt, $proto ); 
				if( $service_guess == "" ){ $service_guess = "<a target=new href=\"http://isc.sans.org/port_details.php?port=$dstprt\">???</a>"; }
				print "<li><a href=\"$url/$this?f=pdrill&q=$dstprt&proto=$proto&d=$q&src=ext\">$dstprt/$proto</a> (<a target=new href=http://isc.sans.org/port_details.php?port=$dstprt>$service_guess</a>) - $dstprt_ct</li>\n";
			}
			print "</ol>";
			...
			{
				print "<li><a href=\"$url/$this?f=drilldown&q=$ext_ip&d=$q\">$ext_ip</a> [<a href=\"javascript:onclick=popupwindow('$url/resolve.php?query=$ext_ip', 'dns', '425', '400' )\">dns</a> | <a href=\"javascript:onclick=popupwindow('$url/portscan.php?query=$ext_ip', 'scan', '425', '400' )\">scan</a> | <a href=\"javascript:onclick=popupwindow('$url/shun.php?query=$ext_ip', 'shun', '425', '400' )\">shun</a> ] ($ext_ct)</li>\n";
			}
...
$tday = today();
print "<br><br><br><div class=\"footer\">$tday -- $query_count queries executed in $time seconds</div>F.W.A.P. v.$version\n";
