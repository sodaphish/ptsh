...
#	TCXO product line.  See "Documentation/Parametric Search" for
...
sub TCXOSearch {
	LoadTemplate( "content/tcxo-search_results.con" );
	...
	$main::httpHeaders .= "Family = $main::__fam}LowTemp = $main::__tltemp}HightTemp = $main::__utemp}Stability = $main::__stability}OutputLogic = $main::__outlogic}SupplyVoltage = $main::__supvolt}PackageType = $main::__package_type";
	Tokenize( 'HTTP_HEADERS', $main::httpHeaders );

	#
	# get the matches and put them in the array @matches.
	@matches = TCXOs_SearchSelect(); 
	...
	if( $NumOfMatches != 0 ){
		...
		TCXOs_RefineSelect( @matches );

		#
		# output the initial headers of the search...
		Parse( "EDIT_SEARCH", "RESULTS_HEADER" );
 
		my $sqlstmt = "select * from ps_tcxos where ( id='$windowMatches[0]'";
		for( my $i = 1; $i < scalar( @windowMatches ); $i++ ){
			$sqlstmt .= " or id='$windowMatches[$i]'";
		} $sqlstmt .= " )";

		my $dispStmt_h = $main::__dsn->prepare( $sqlstmt )
			or NewLog( 'wrn', '0A', "Search() - Could not PREPARE '$sqlstmt'.  ($DBI::errstr)" );
		$dispStmt_h->execute
			or NewLog( 'err', '0A', "Search() - Could not EXECUTE '$sqlstmt'. ($DBI::errstr)" );

		my $white = 0;

		while( my( $id, $fam, $par, $lte, $ute, $sta, $fre, $sym, $log, $pckg, $sup, $pdf, 
...
sub TCXOs_RefineSelect {

	local @matches = @_;

	my $sqlstmt = "select family, ltemp, utemp, stability, symmetry, logic, supply_voltage, pckg_info from ps_tcxos where ( id='$matches[0]'";
	...
	while( my( $fam, $lte, $ute, $sta, $sym, $log, $sup, $pck ) = $editStmt_h->fetchrow() ){
...
} #end TCXOs_RefineSelect()



sub TCXOs_SearchSelect {
	my @matches = ();

	my $sqlstmt = "select id from ps_tcxos where ( ( ( freq_lower <= $main::__frequency and freq_upper >= $main::__frequency and freq_std is null ) or (freq_std = $main::__frequency) ) and pckg = '$main::__mounting' ";
